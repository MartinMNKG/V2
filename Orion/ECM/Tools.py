...
def Change_detailed(time_list,temp_list,Y_target_list,Y_non_target_list,case,target,non_target,param):
    ...
    if info_shift == "shift":
        time_list_scl,time_list_shift = shift(time_list, temp_list,case)
    
    if info_interp =="interpol" : 
        Y_target_list = interp_Y(time_list_shift,Y_target_list,time_list_scl,case,target)
            
        Y_non_target_list = interp_Y(time_list_shift,Y_non_target_list,time_list_scl,case,non_target)   
        
        temp_list = interp_T(time_list_shift,temp_list,time_list_scl,case)
    ...
    return time_list_scl,temp_list,Y_target_list,Y_non_target_list,Y_scaler_Targets,Y_scaler_Non_Targets, ai_delay,Temp_scaler 
...
def Change_Reduced(time_list,temp_list,Y_target_list,Y_non_target_list,case,target,non_target,Time_Det,Scal_Target,Scal_Non_Target,Temp_scaler,param):
    ...
    if info_shift == "shift":
        _,time_list_shift = shift(time_list, temp_list,case)
    
    if info_interp =="interpol" : 
        Y_target_list = interp_Y(time_list_shift,Y_target_list,Time_Det,case,target)
            
        Y_non_target_list = interp_Y(time_list_shift,Y_non_target_list,Time_Det,case,non_target)   
        
        temp_list = interp_T(time_list_shift,temp_list,Time_Det,case)
    ...
    return Time_Det,temp_list,Y_target_list,Y_non_target_list , ai_delay 
...
def shift(time_l, temp_l,case) :
    ...
    for c in range(len(case)) : 
        ...
        shift_time_scl.append(np.linspace(min(shift_grid),max(shift_grid),500))
        
    return shift_time_scl,shift_time
...
def fit_orch(Y_t_red,Y_t_det,coef,species,eps,coef_non_target=0.05): 

    Abs_err = np.abs(np.array(Y_t_det)-np.array(Y_t_red))/np.abs(Y_t_det)
    mask = np.abs(Y_t_det )< eps
    Abs_err[mask] = 0 
    err_spec =[]
    for spec in species : 
        ind = species.index(spec)
        k = coef.get(spec,coef_non_target)
        err_spec.append(k*np.sum(Abs_err[:,ind,:].flatten()))
    return np.sum(err_spec), 
