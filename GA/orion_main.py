...
def optim_prob(pop_size,ngen,mutpb,cxpb,elitism_size,file_detailed,file_reduced,fitness,mode) :
    ...
    temperature = np.linspace(1000,2000,10).tolist()
    phi = np.linspace(0.7,2.0,10).tolist()
    mixture =np.linspace(0.85,0.85,1).tolist() 
    Targets = ["H2", "NH3", "O2", "OH","NO","NH","NH2","NNH", 'H2O','NO2', 'N2O']
    Non_Target = ['N2', 'H', 'O', 'HO2', 'N', 'N2H3', 'N2H2', 'HNO']
